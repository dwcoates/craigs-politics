...
*** Words                                                            :ignore:
    ...
    :END:
We find that "against", "how", and "won" have extreme preference for "liberal"
states. Some random sampling of such posts:
#+BEGIN_SRC ipython :session :results output raw drawer :noweb yes  :exports results
  print_df(pd.DataFrame(pd.concat([find_strs("against"),
                                   find_strs("usa"),
...
|       | title                                                                  |
|-------+------------------------------------------------------------------------|
|  3453 | RE Proof of massive Democrats voter fraud thousands voter fraud ..     |
| 27278 | Lightworkers united against Greed                                      |
|  9547 | re: Tens of thousands more Mexicans flooding across Texas's border.    |
| 13873 | RUSSIANS DO NOT WANT WAR WITH USA OR NATO DO NOT BELIEVE LIES AND PROP |
|  1994 | RE: Bakery won't...............                                        |
:END:
...
ratio of upper to proper. It isn't too surprising, as you expect, in leiu of
social resistance, for pro-Trump regions to settle for respectful praise over
excitement and hysteria, a general tone which is supported by some cursory
sampling of these regions.
...
respectively, are comparitively not quite as distinguished. This is expected,
for previously mentioned reasons; pluralizations may still be used as a means to
negatively generalize.
#+BEGIN_SRC ipython :session :exports results :results output org :noweb yes
...
than twice as much as the pluralization. I suspect this is because "liberal" is
a perjorative in common nomenclature, while "conservative" doesn't really hold
the same weight as an insult:
#+BEGIN_SRC ipython :session :exports results :results output org :noweb yes
...
**** *Capitalization*
We here see that, among democrats, "liberal" is capitalized at a rate 13x
greater than the rate of capitalization of "conservative". We also see that
lowecase usage preference is completely neglible.
#+BEGIN_SRC ipython :session :exports code
